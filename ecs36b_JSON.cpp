...
(char *f_name) //passes file name
{
  ...
  long lSize;
  size_t lresult;
//seeks from the beginning to end of the file
  fseek(f_ptr, 0 , SEEK_END);
  //tells how many bytes from the beginning of file (size of file)
  lSize = ftell(f_ptr);
  //f_ptr(the offset) goes back to zero, goes back to beginning of file
  rewind(f_ptr);
  ...
  //checks whether entire file has been read
  if (lresult != lSize)
    {
